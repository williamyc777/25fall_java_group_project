...
### 4.5 Advanced Capabilities (Highlight)
- **Spring Boot + Maven**: Dependency/build lifecycle, profile-based config (dev H2 file DB, prod MySQL).
- **Network Security**: JWT auth, role/permission checks (Admin/User; canCreate/canEnroll/canComment), interceptors, token verification in controllers.
- **JPA/Hibernate**: Transactional boundaries, JOIN FETCH optimization, cache control, lazy-loading handling for complex relations.
- **Multithreading**: Tomcat thread pool serves concurrent HTTP/WebSocket sessions; transactional consistency for enroll/like/collect flows.
- **Testing**: JUnit5 + Spring Boot Test + MockMvc smoke tests (main page availability, admin login with JWT, signup/login with hashed passwords).
